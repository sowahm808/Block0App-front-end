...
@Component({
  selector: 'b0-reward-empty-state',
  standalone: true,
  imports: [AsyncPipe, JsonPipe, PageHeaderComponent, LoadingSkeletonComponent, EmptyStateComponent, ErrorStateComponent],
  template: `<b0-page-header title="Reward Empty State" description="Production component for Block Zero workflows." />
  @if (state$ | async; as state) {
    ...
    @else { <section class="feature-grid" tabindex="0" aria-label="Reward Empty State content"><pre>{{ state.data | json }}</pre></section> }
  }`,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class RewardEmptyStateComponent {
