...
@Component({
  selector: 'b0-approval-action-bar',
  standalone: true,
  imports: [AsyncPipe, JsonPipe, PageHeaderComponent, LoadingSkeletonComponent, EmptyStateComponent, ErrorStateComponent],
  template: `<b0-page-header title="Approval Action Bar" description="Production component for Block Zero workflows." />
  @if (state$ | async; as state) {
    ...
    @else { <section class="feature-grid" tabindex="0" aria-label="Approval Action Bar content"><pre>{{ state.data | json }}</pre></section> }
  }`,
