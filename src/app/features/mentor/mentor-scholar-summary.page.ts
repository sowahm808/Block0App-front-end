...
@Component({
  selector: 'b0-mentor-scholar-summary', standalone: true,
  ...
  template: `<nav class="mb-4 text-sm"><a mat-button routerLink="/mentor/teams">← Back</a></nav>
    <b0-page-header title="Scholar summary" description="Loads this detail page with a resolved route identifier before calling the backend." />
    @if (state$ | async; as state) {
      ...
      @else { <b0-data-template [data]="state.data" ariaLabel="Scholar summary content" /> }
    } @else { <b0-empty-state title="Scholar summary is starting" message="The page will show a safe error instead of going blank if the record cannot be loaded." /> }`,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class MentorScholarSummaryPage {
  ...
  readonly state$ = this.#route.paramMap.pipe(switchMap((params) => {
    const id = params.get('scholarId');
    if (!id) return of({ status: 'error', message: 'Missing required route id: scholarId.' } satisfies ApiState<unknown>);
    return this.#api.get<unknown>('/mentor/scholars/{scholarId}'.replace('{scholarId}', encodeURIComponent(id))).pipe(
