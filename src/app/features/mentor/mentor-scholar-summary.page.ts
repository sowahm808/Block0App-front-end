...
@Component({
  selector: 'b0-mentor-scholar-summary',
  standalone: true,
  imports: [AsyncPipe, JsonPipe, PageHeaderComponent, LoadingSkeletonComponent, EmptyStateComponent, ErrorStateComponent],
  template: `<b0-page-header title="Mentor Scholar Summary" description="Production page for Block Zero workflows." />
  @if (state$ | async; as state) {
    ...
    @else { <section class="feature-grid" tabindex="0" aria-label="Mentor Scholar Summary content"><pre>{{ state.data | json }}</pre></section> }
  }`,
