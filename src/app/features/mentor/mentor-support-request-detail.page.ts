...
@Component({
  selector: 'b0-mentor-support-request-detail', standalone: true,
  ...
  template: `<nav class="mb-4 text-sm"><a mat-button routerLink="/mentor/support-requests">← Back</a></nav>
    <b0-page-header title="Support request" description="Loads this detail page with a resolved route identifier before calling the backend." />
    @if (state$ | async; as state) {
      ...
      @else { <b0-data-template [data]="state.data" ariaLabel="Support request content" /> }
    } @else { <b0-empty-state title="Support request is starting" message="The page will show a safe error instead of going blank if the record cannot be loaded." /> }`,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class MentorSupportRequestDetailPage {
  ...
  readonly state$ = this.#route.paramMap.pipe(switchMap((params) => {
    ...
    if (!id) return of({ status: 'error', message: 'Missing required route id: requestId.' } satisfies ApiState<unknown>);
    return this.#api.get<unknown>('/mentor/support-requests/{requestId}'.replace('{requestId}', encodeURIComponent(id))).pipe(
