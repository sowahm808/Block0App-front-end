...
@Component({
  selector: 'b0-mentor-team-detail', standalone: true,
  ...
  template: `<nav class="mb-4 text-sm"><a mat-button routerLink="/mentor/teams">← Back</a></nav>
    <b0-page-header title="Mentor team detail" description="Loads this detail page with a resolved route identifier before calling the backend." />
    @if (state$ | async; as state) {
      ...
      @else { <b0-data-template [data]="state.data" ariaLabel="Mentor team detail content" /> }
    } @else { <b0-empty-state title="Mentor team detail is starting" message="The page will show a safe error instead of going blank if the record cannot be loaded." /> }`,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class MentorTeamDetailPage {
  ...
  readonly state$ = this.#route.paramMap.pipe(switchMap((params) => {
    ...
    if (!id) return of({ status: 'error', message: 'Missing required route id: teamId.' } satisfies ApiState<unknown>);
    return this.#api.get<unknown>('/mentor/teams/{teamId}'.replace('{teamId}', encodeURIComponent(id))).pipe(
