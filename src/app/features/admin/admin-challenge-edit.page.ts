...
@Component({
  selector: 'b0-admin-challenge-edit',
  standalone: true,
  imports: [AsyncPipe, JsonPipe, PageHeaderComponent, LoadingSkeletonComponent, EmptyStateComponent, ErrorStateComponent],
  template: `<b0-page-header title="Admin Challenge Edit" description="Production page for Block Zero workflows." />
  @if (state$ | async; as state) {
    ...
    @else { <section class="feature-grid" tabindex="0" aria-label="Admin Challenge Edit content"><pre>{{ state.data | json }}</pre></section> }
  }`,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class AdminChallengeEditPage {
