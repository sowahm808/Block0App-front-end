...
@Component({
  selector: 'b0-admin-question-edit',
  standalone: true,
  imports: [AsyncPipe, JsonPipe, PageHeaderComponent, LoadingSkeletonComponent, EmptyStateComponent, ErrorStateComponent],
  template: `<b0-page-header title="Admin Question Edit" description="Production page for Block Zero workflows." />
  @if (state$ | async; as state) {
    ...
    @else { <section class="feature-grid" tabindex="0" aria-label="Admin Question Edit content"><pre>{{ state.data | json }}</pre></section> }
  }`,
