...
@Component({
  selector: 'b0-scenario-timer',
  standalone: true,
  imports: [AsyncPipe, JsonPipe, PageHeaderComponent, LoadingSkeletonComponent, EmptyStateComponent, ErrorStateComponent],
  template: `<b0-page-header title="Scenario Timer" description="Production component for Block Zero workflows." />
  @if (state$ | async; as state) {
    ...
    @else { <section class="feature-grid" tabindex="0" aria-label="Scenario Timer content"><pre>{{ state.data | json }}</pre></section> }
  }`,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class ScenarioTimerComponent {
