...
@Component({
  selector: 'b0-support-request-status',
  standalone: true,
  imports: [AsyncPipe, JsonPipe, PageHeaderComponent, LoadingSkeletonComponent, EmptyStateComponent, ErrorStateComponent],
  template: `<b0-page-header title="Support Request Status" description="Production component for Block Zero workflows." />
  @if (state$ | async; as state) {
    ...
    @else { <section class="feature-grid" tabindex="0" aria-label="Support Request Status content"><pre>{{ state.data | json }}</pre></section> }
  }`,
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class SupportRequestStatusComponent {
