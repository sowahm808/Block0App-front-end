...
@Component({
  selector: 'b0-edit-profile-form',
  standalone: true,
  imports: [AsyncPipe, JsonPipe, PageHeaderComponent, LoadingSkeletonComponent, EmptyStateComponent, ErrorStateComponent],
  template: `<b0-page-header title="Edit Profile Form" description="Production component for Block Zero workflows." />
  @if (state$ | async; as state) {
    ...
    @else { <section class="feature-grid" tabindex="0" aria-label="Edit Profile Form content"><pre>{{ state.data | json }}</pre></section> }
  }`,
