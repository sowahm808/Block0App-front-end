...
export class AuthStore {
  ...
  hasRole(roles: UserRole[]) {
    const u = this.user();
    return !!u && roles.some((r) => u.roles.includes(r) || u.roles.includes('SuperAdministrator'));
  }
  hasPermission(perms: Permission[]) {
    const u = this.user();
    return !!u && perms.every((p) => u.permissions.includes(p) || u.roles.includes('SuperAdministrator'));
  }
}
