...
export class FirebaseAuthService {
  ...
  signInWithPassword(
    ...
    return this.#emailPasswordRequest(
  ...
  signUpWithPassword(
    ...
    return this.#emailPasswordRequest(
      ...
      switchMap((idToken) =>
        this.sendEmailVerification(idToken).pipe(
          map(() => idToken),
        ),
      ),
    );
  ...
  signInWithCustomToken(
    ...
    return this.#post<FirebaseAuthResponse>(
      ...
      map((response) =>
        this.#setFirebaseSession(response),
      ),
    );
  }
  ...
  sendEmailVerification(
    ...
    if (!idToken) {
      return throwError(
        () => new Error(
          'A Firebase ID token is required to send the verification email.',
        ),
      );
    }

    return this.#post<FirebaseVerificationResponse>(
  ...
  resendEmailVerification(
    ...
    return this.signInWithPassword(
      ...
      switchMap((idToken) =>
        this.sendEmailVerification(idToken),
      ),
    );
  }
  ...
  refreshIdToken(): Observable<string> {
    ...
    return this.#http
      .post<FirebaseAuthResponse>(
        `${environment.firebase.secureTokenUrl}/token?key=${environment.firebase.apiKey}`,
        {
